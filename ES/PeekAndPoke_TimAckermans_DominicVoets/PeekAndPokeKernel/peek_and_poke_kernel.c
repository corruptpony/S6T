...
{
    int i;
    char command = 'x'; /* If command wasn't changed from x, something went wrong */
    unsigned int addr = 0;
    ...
    sscanf(buffer, "%c %x %x", &command, &addr, &value);
    ...
    /* Check if the command is read (r), write (w), or invalid */
    if(command == 'r')
    {
