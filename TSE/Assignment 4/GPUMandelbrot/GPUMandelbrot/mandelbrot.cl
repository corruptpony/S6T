...
)
{
	...
	*/
	...
	__local float x = 0.0;
	__local float y = 0.0;
	__local float xSqr = 0.0;
	__local float ySqr = 0.0;
	...
	mandelbrot_color output = (iterations == PARAMS[5]) ? black : colortable[iterations];
	int2 coord = (int2)windowPosX, windowPosY);
	floa4 color_yellow = (float4)(output.red / 255, output.green / 255, output.blue / 255, 1.0);
	write_imagef(buffer, coord, color_yellow);