...
//!	Generated Date	: Mon, 8, May 2017  
	File Path	: PhytecTest/VirtualConfig/Tacho.cpp
*********************************************************************/
...
Tacho::Tacho(IOxfActive* theActiveContext) : interval(100) {
...
IOxfReactive::TakeEventStatus Tacho::rootState_processEvent() {
    ...
    switch (rootState_active) {
        case Idle:
        {
            if(IS_EVENT_TYPE_OF(evStartTacho_Design_id))
                {
        ...
        case Running:
        {
            ...
            else if(IS_EVENT_TYPE_OF(evStopTacho_Design_id))
                {
                    if(Running_timeout != NULL)
                        {
                            Running_timeout->cancel();
